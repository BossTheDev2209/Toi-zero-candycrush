...
export function ProblemWorkspacePage() {
  ...
  // Signature of the last-persisted (lang, code). null until the solution loads,
  // which gates auto-save so it never fires with stale or pre-hydration content.
  const savedSigRef = useRef<string | null>(null);
  const sigOf = (l: Language, c: string) => `${l} ${c}`;
  // Results render full-width below the grid now; scroll them into view on a run.
  ...
  useEffect(() => {
    ...
    setSubmitLink(null);
    setImportMsg(null);
    // Block auto-save until this problem's solution has hydrated, so we never
    // persist the previous problem's code under the new id.
    ...
    api.getSolution(pid).then((s) => {
      if (cancelled) return;
      const loadedLang = s ? s.language : "cpp";
      const loadedCode = s ? s.code : starterFor("cpp");
      if (s) setLang(s.language);
      setCode(loadedCode);
      savedSigRef.current = sigOf(loadedLang, loadedCode);
    });
    api.listRuns(pid).then((r) => { if (!cancelled) setRuns(r); });
    return () => { cancelled = true; };
  }, [pid]);

  // Debounced auto-save: persist the editor a beat after edits settle. Gated on
  // savedSigRef so it skips the initial hydration and unchanged content.
  useEffect(() => {
    if (savedSigRef.current === null) return;
    if (sigOf(lang, code) === savedSigRef.current) return;
    const t = setTimeout(async () => {
      try {
        await api.saveSolution(pid, lang, code);
        savedSigRef.current = sigOf(lang, code);
        setSavedMsg("Saved"); setTimeout(() => setSavedMsg(null), 1200);
      } catch { /* transient — next edit or manual Save retries */ }
    }, 1000);
  ...
  return (
    <div className="pt-28 px-6 max-w-[1600px] mx-auto">
      ...
      {/* Test results live full-width below the editor so they never compress the
          coding area. Auto-scrolled into view after a run for immediate feedback. */}
      {result && (
        <section ref={resultsRef} className="mt-10 scroll-mt-28">
          <EyebrowLabel>Test results</EyebrowLabel>
