...
export const VERDICTS: Array<{
  ...
  },
  {
    ...
    fix: "Read the compiler stderr — fix syntax or missing includes.",
...
export function VerdictGlossary({ compact = false }: { compact?: boolean }) {
      ...
      <div className="flex flex-wrap gap-2">
        {VERDICTS.map((v) => (
          <span
            ...
            className="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1 text-xs border border-[var(--color-dust)]"
          >
  ...
  return (
    <div className="rounded-[24px] bg-[var(--color-lifted)] p-6">
      <div className="text-[12px] font-bold tracking-[0.04em] uppercase text-[var(--color-slate)] mb-3">
        Verdicts
      </div>
      <ul className="space-y-3">
        {VERDICTS.map((v) => (
          <li key={v.code} className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--color-slate)] mt-0.5">→ {v.fix}</p>
            </div>
          </li>
        ))}
      </ul>
